...
def get_resources():
    ...
    resources = {
        "RasPi": {
            ...
            "storage_provider_alias": "minio-raspi",
        ...
        "VM1": {
            ...
            "storage_provider": "minio",
            "storage_provider_alias": "minio-vm",
...
# todo move to reading from file
def get_testing_parameters():
    ...
    keys = list(testing_parameters.keys())
    length = len(testing_parameters[keys[0]]["parallelism"])

    for k in keys:
        if len(testing_parameters[k]["parallelism"]) != length:
            show_fatal_error("Parallelism lists of different length in file \"testing_parameters.yaml\"")
