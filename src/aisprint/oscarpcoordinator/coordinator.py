...
def main(work_dir):
    ...
    for i in range(len(deployments)):
        ...
        current_deployment_dir = campaign_dir + "/deployments/" + "deployment_" + str(i)

        make_oscar_p_input_file(d, testing_components, resources, run_parameters, images,
                                is_single_service=False, service_number=0)

        oscarp.main(clean_buckets=True, work_dir=work_dir + "oscarp/")

        auto_mkdir(current_deployment_dir + "/single_services/")
        services_in_deployment = get_single_services_from_deployment(d)
        ...
        for j in range(len(services_in_deployment)):
            ...
            oscarp.main(work_dir=work_dir + "oscarp/")
            print()
