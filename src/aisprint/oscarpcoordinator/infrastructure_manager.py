...
# todo rename all as infrastructure
...
def create_virtual_infrastructures():
    ...
    print(gp.virtual_infrastructures)
...
def _generate_modified_candidate_files(virtual,):
    ...
    with open("%stoscarizer/modified_candidate_deployments.yaml" % gp.current_deployment_dir, 'w') as file:
...
def generate_tosca():
    elastic = 0
    auth_data = None
    with open("%s%s" % (gp.application_dir, CONTAINERS_FILE), 'r') as f:
    ...
    for cl, tosca in toscas.items():
        tosca_file = "%stoscarizer/%s.yaml" % (gp.current_deployment_dir, cl)
