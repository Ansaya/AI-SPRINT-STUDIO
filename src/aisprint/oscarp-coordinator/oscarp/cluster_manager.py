...
def remove_all_services(clusters):
    ...
    for c in clusters:
        ...
        command = oscar_cli + "service ls"
        services = get_command_output_wrapped(command)
        services.pop(0)
        for service in services:
            service = service.split()[0]
            command = oscar_cli + "service remove " + service
...
def create_bucket(bucket, cluster_name, clusters):
    ...
    minio_alias = cluster["minio_alias"]
    command = mc + "mb " + minio_alias + "/" + bucket
    get_command_output_wrapped(command)
    print(colored("Done!", "green"))
    return
...
def clean_all_logs():
    ...
    for service in services:
        ...
        command = oscar_cli + "service logs remove " + service + " --all"
        get_command_output_wrapped(command)
        print(colored("Done!", "green"))
...
def generate_fdl_configuration(services, clusters):
    with open("oscarp/FDL_configuration.yaml", "w") as file:
...
def _apply_fdl_configuration():
    command = oscar_cli + "apply oscarp/FDL_configuration.yaml"
    get_command_output_wrapped(command)
    return
...
def get_deployed_services(oscarcli_alias):
    ...
    command = oscar_cli + "service list"
...
def set_default_oscar_cluster_from_alias(oscarcli_alias):
    command = oscar_cli + "cluster default -s " + oscarcli_alias
    get_command_output_wrapped(command)
